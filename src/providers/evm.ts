...
export class Evm {
  ...
  erc20Contract(
    ...
    multicall = false,
    includeUniV2 = false
  ): ethers.Contract | MulticallContract {
    const abi = [
      Abi.Transfer,
      Abi.BalanceOf,
      Abi.TotalSupply,
    ...
    return multicall
      ? new MulticallContract(token, abi)
      : new ethers.Contract(token, abi, this.provider);
  }
  ...
  async getUniV2Holdings(
    ...
    for (const { address, amount } of balances) {
      ...
      const perc = (amount as ethers.BigNumber)
        .mul(10 ** 8)
        .div(totalSupply.div(100));
  ...
  async getErc20Balances(
    ...
    const balances = await ethcallProvider.all(
      Array.from(addresses).map((address) => contract.balanceOf(address))
      // block
    );
    ...
    for (const address of addresses) {
      ...
      if (balance.toString() !== "0") {
        result.add({
          address,
          amount: stringifyBalances ? balance.toString() : balance,
        });
      }
    }
  ...
  async listenErc20Transfers(
    ...
    const listener = async (
      ...
      await callback(
        Evm.wrapTransfer(from, to, amount, denomination, blockNumber)
      );
