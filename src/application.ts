...
export class Application {
  ...
  async indexUniV2PoolPromised(
    stringifyBalances = false
  ): Promise<UniV2HoldingsResponse> {
    return new Promise(async (resolve) => {
      await this.indexUniV2Pool(
        (holdings) => resolve(holdings),
        stringifyBalances
      );
    });
  }

  async indexUniV2Pool(
    onEnd?: Function,
    stringifyBalances = false
  ): Promise<Function> {
    return this.index(
      async (balances) => {
        ...
        this.logging &&
          this.log.darkGray(
            `Calculating ${balances.size} LPs holdings in "${this.config.get(
              "token"
            )}" Uni V2 pool`
          );
      ...
      true,
      false
    ); // doesn't make sense if no balances read
  }
  ...
  async index(
    ...
    this.logging &&
      this.log.green(`DENOMINATION=${this.config.get("denomination")}`);
      ...
      async (event) => {
        if (event == EndOfEvents) {
          ...
          if (readBalances) {
            ...
            this.logging &&
              this.log.darkGray(
                `Read balances of ${uniqueAddresses.size} addresses`
              );
            ...
            this.logging &&
              this.log.darkGray(
                `Found ${balances.size} addresses w/ positive balances`
              );
          }
        ...
        this.logging &&
          this.log.yellow(
            `[${event.blockNumber}] ${event.type}(${event.fromShort}, ${event.toShort}, ${event.amountHuman})`
          );
